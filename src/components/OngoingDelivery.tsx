...
import {
  ...
  assignPackageToUser,
  updatePackageStatusToDelivered,updatePackageStatusToCancelled
...
const RepartoEnCurso: React.FC = () => {
  ...
  //         onClick={() => {
  //           if (packageInfo && packageInfo.status === "PENDIENTE") {
    ...
    <div className="flex items-center justify-center flex-wrap  rounded-xl mx-[30px] mt-[25px] mb-20 bg-[#C7FFB1] relative ">
      ...
      <div className="bg-white rounded-xl z-20 text-[#3d1df3] text-[12px] w-[19rem] pl-4">
        ...
        {packageInfo && packageInfo.status === "CANCELADO" ? (
          <p className="text-[#FF0000] text-center mt-6 mb-4">
            Cancelado
          </p>
        ) : (
          <button
        ...
        >
          {packageInfo && packageInfo.status === "PENDIENTE" ? "Iniciar" : "Finalizar"}
        </button>
        )}
      </div>
      {packageInfo && packageInfo.status !== "CANCELADO" && (
